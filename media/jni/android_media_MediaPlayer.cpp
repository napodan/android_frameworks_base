...
android_media_MediaPlayer_setDataSourceAndHeaders(
    ...
    ALOGV("setDataSource: path %s", pathStr);
    status_t opStatus =
        mp->setDataSource(
                String8(pathStr),
...
android_media_MediaPlayer_prepareAsync(JNIEnv *env, jobject thiz)
{
    ...
    if (surface != NULL) {
        const sp<Surface> native_surface = get_surface(env, surface);
        ALOGV("prepareAsync: surface=%p (id=%d)",
             native_surface.get(), native_surface->getIdentity());
        mp->setVideoSurface(native_surface);
    }
...
android_media_MediaPlayer_native_suspend_resume(
    ...
    ALOGV("suspend_resume(%d)", isSuspend);
    sp<MediaPlayer> mp = getMediaPlayer(env, thiz);
    if (mp == NULL ) {
        jniThrowException(env, "java/lang/IllegalStateException", NULL);
        return UNKNOWN_ERROR;
...
jint JNI_OnLoad(JavaVM* vm, void* reserved)
{
    ...
    if (register_android_media_MtpClient(env) < 0) {
        ALOGE("ERROR: MtpClient native registration failed");
        goto bail;
    }

    if (register_android_media_MtpCursor(env) < 0) {
        ALOGE("ERROR: MtpCursor native registration failed");
        goto bail;
    }
