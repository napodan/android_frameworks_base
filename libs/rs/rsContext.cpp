...
void Context::setSurface(uint32_t w, uint32_t h, ANativeWindow *sur)
{
    ...
    if (mWndSurface != NULL) {
        ...
        if ((int)mWidth != mEGL.mWidth || (int)mHeight != mEGL.mHeight) {
            ALOGE("EGL/Surface mismatch  EGL (%i x %i)  SF (%i x %i)", mEGL.mWidth, mEGL.mHeight, mWidth, mHeight);
        }
...
void Context::objDestroyOOBRun()
{
    if (mObjDestroy.mNeedToEmpty) {
        ...
        if (status) {
            ALOGE("Context::ObjDestroyOOBRun: error %i locking for OOBRun.", status);
            return;
        }
        ...
        if (status) {
            ALOGE("Context::ObjDestroyOOBRun: error %i unlocking for set condition.", status);
        }
    }
}
...
void Context::dumpDebug() const
{
    ...
    ALOGE(" EGL context %p  surface %p,  w=%i h=%i  Display=%p", mEGL.mContext,
