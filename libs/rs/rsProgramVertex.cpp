...
{
    ALOGV("Matrix %s, %p", txt, f);
    ALOGV("%6.4f, %6.4f, %6.4f, %6.4f", f[0], f[4], f[8], f[12]);
    ALOGV("%6.4f, %6.4f, %6.4f, %6.4f", f[1], f[5], f[9], f[13]);
    ALOGV("%6.4f, %6.4f, %6.4f, %6.4f", f[2], f[6], f[10], f[14]);
    ALOGV("%6.4f, %6.4f, %6.4f, %6.4f", f[3], f[7], f[11], f[15]);
}
...
void ProgramVertex::setupGL(const Context *rsc, ProgramVertexState *state)
{
    ...
    if (!f) {
        ALOGE("Must bind constants to vertex program");
    }
...
void ProgramVertex::setupGL2(const Context *rsc, ProgramVertexState *state, ShaderCache *sc)
{
    ...
    for (uint32_t ct=0; ct < mConstantCount; ct++) {
        ...
        for (uint32_t field=0; field < e->getFieldCount(); field++) {
            ...
            //ALOGE("Uniform  slot=%i, offset=%i, constant=%i, field=%i, uidx=%i", slot, offset, ct, field, uidx);
            if (slot >= 0) {
                switch(f->getComponent().getVectorSize()) {
                case 1:
                    //ALOGE("Uniform 1 = %f", fd[0]);
                    glUniform1fv(slot, 1, fd);
                    break;
                case 2:
                    //ALOGE("Uniform 2 = %f %f", fd[0], fd[1]);
                    glUniform2fv(slot, 1, fd);
                    break;
                case 3:
                    //ALOGE("Uniform 3 = %f %f %f", fd[0], fd[1], fd[2]);
                    glUniform3fv(slot, 1, fd);
                    break;
                case 4:
                    //ALOGE("Uniform 4 = %f %f %f %f", fd[0], fd[1], fd[2], fd[3]);
