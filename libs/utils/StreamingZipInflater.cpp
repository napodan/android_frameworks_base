...
void StreamingZipInflater::initInflateState() {
    ALOGD("Initializing inflate state");
...
ssize_t StreamingZipInflater::read(void* outBuf, size_t count) {
    ...
    while (toRead > 0) {
        ...
        if (toRead > 0) {
            ...
            /*
            ALOGD("Inflating to outbuf: avail_in=%u avail_out=%u next_in=%p next_out=%p",
                    mInflateState.avail_in, mInflateState.avail_out,
                    mInflateState.next_in, mInflateState.next_out);
            */
            int result = Z_OK;
            if (mStreamNeedsInit) {
                ALOGD("Initializing zlib to inflate");
...
int StreamingZipInflater::readNextChunk() {
    ...
    if (mInNextChunkOffset < mInTotalSize) {
        ...
        if (toRead > 0) {
            ...
            //ALOGD("Reading input chunk, size %08x didread %08x", toRead, didRead);
            if (didRead < 0) {
                // TODO: error
